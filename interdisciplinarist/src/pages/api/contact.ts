...
export const POST: APIRoute = async ({ request }) => {
  ...
  const { error } = await resend.emails.send({
    from: 'Interdisciplinarist <hello@interdisciplinarist.com>',
    ...
    html: `
      <table style="font-family:sans-serif;font-size:15px;line-height:1.7;color:#2c2b29;max-width:580px;border-collapse:collapse">
        <tr><td style="padding:0 0 20px">
          <h2 style="margin:0;font-size:20px;color:#1b6b7a">New contact — Interdisciplinarist</h2>
        </td></tr>
        ...
        <tr><td style="padding:4px 0"><strong>Topic:</strong> ${topicLabel}</td></tr>
